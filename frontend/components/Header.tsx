...
// Three dots image data (for 3. button)
    ...
    <div className={styles.header}>
      ...
      <div className={styles.header__actions}>
        ...
        {!user ? (
          ...
          <>
            <button disabled style={{ color: "gray", background: "silver" }}>
              {!address
                ? // If not connected, render connect wallet
                "Connect Wallet"
                : // Else, render address
            ...
            <Link href="/api/logout">
              <button style={{ color: "red", background: "silver" }}>{user.publicAddress.substr(0, 6)}...
                {user.publicAddress.slice(user.publicAddress.length - 4)}</button>
            </Link>
          </>
        )}
